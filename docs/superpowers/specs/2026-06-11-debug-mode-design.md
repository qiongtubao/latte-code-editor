...
| `src/utils/debug/inject.ts` | `debugEmit`、`registerDebugEvent`、`lastAction` 栈 |
| `src/utils/debug/store.ts` | `useDebugStore`（开关、`isOn`、会话 id） |
| `src/components/DebugBar.tsx` | 右下角调试栏 + 抽屉 |
| `src/components/DebugEventInjectModal.tsx` | 事件注入面板 |
...
| 路径 | 改动 |
|-----|------|
| `src/App.tsx` | 加载 `useDebugStore`、渲染 `DebugBar`、处理 `Ctrl+Shift+D/R/S/E` |
| `src/hooks/useLspStore.ts` | 操作前/后调 `debugEmit`、记 `lastAction` |
| `src/hooks/useWorkspaceStore.ts` | 同上 |
| `src/hooks/useEditorStore.ts` | 同上 |
| `src/hooks/useGraphStore.ts` | 同上 |
| `src/api/lsp.ts` | invoke 包装器自动记录 `ipc.*` |
| `src-tauri/src/lib.rs` | 初始化 debug 模块（读 `LATTE_DEBUG` env） |
| `src-tauri/src/editor/lsp_commands.rs` | 所有 command 加 `tracing::info!` |
...
- 日志中**禁止**记录：文件内容（`content` 字段）、用户输入的密码、API key
- 自动清理：超过 7 天的日志文件启动时清理
...
- `logger.test.ts`：写一行 JSON 行可被解析；采样/节流生效
- `useDebugStore.test.ts`：开关切换、`LATTE_DEBUG=1` 自动开启
- `inject.test.ts`：`debugEmit` 被注册的事件收到正确 ctx
...
- 任何用户操作没有对应的 event 埋点
- 关闭 Debug 后文件仍在增长
- 任何发布包默认就打开了 Debug
- 切换 Workspace 30 秒内没出现完整 ws.activate 链路
...
| 日志泄漏敏感信息 | 显式黑名单 + content 字段永不记录 |
| 事件注入误触真实破坏 | 注入事件走与正常路径相同的 API，不绕过安全检查 |
| 用户嫌"日志文件占空间" | 7 天自动清理 + 单文件 5MB 滚动 |
