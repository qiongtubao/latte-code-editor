export interface DocSimNode {
  ...
  group: number; label: string; docType: string; path: string;
}
...
interface DocFile {
  name: string; path: string; outLinks?: string[]; docType?: string;
...
const KIND_WEIGHT: Record<string, number> = {
  "source-shared": 1.0, wikilink: 0.75, "same-group": 0.4,
};

function topGroup(id: string): string { const p = id.split("/"); return p.length > 1 ? p[0] : "(root)"; }
function basename(id: string): string { return id.split("/").filter(Boolean).pop() ?? id; }
...
export async function buildDocSim(
  ...
  for (const f of mdFiles) {
    ...
    try {
      ...
      parsed.set(id, { name: f.name, path: f.path, outLinks: links, docType: fm.type ?? "other", title: fm.title ?? f.name, sources: fm.sources ?? [] });
    } catch {
  ...
  for (const [id, doc] of parsed) {
    nodes.push({ id, x: cx + Math.random() * 400 - 200, y: cy + Math.random() * 300 - 150, vx: 0, vy: 0, group: TYPE_GROUP[doc.docType ?? "other"] ?? 12, label: doc.title ?? id, docType: doc.docType ?? "other", path: doc.path });
  }

  const edges: DocSimEdge[] = [];
  const seen = new Set<string>();
  const addEdge = (from: string, to: string, kind: string) => {
    if (from === to) return; const key = `${from}→${to}#${kind}`;
    if (seen.has(key)) return; seen.add(key);
    edges.push({ source: from, target: to, kind, weight: KIND_WEIGHT[kind] ?? 0.5 });
  };

  const byBasename = new Map<string, string[]>();
  for (const id of ids) { const b = basename(id); const arr = byBasename.get(b) ?? []; arr.push(id); byBasename.set(b, arr); }

  for (const [fromId, doc] of parsed) {
    if (!doc.outLinks) continue;
    for (const target of doc.outLinks) {
      let resolved: string | null = null;
      for (const c of [target, `${target}.md`, target.replace(/\.md$/, "")]) { if (ids.has(c)) { resolved = c; break; } }
      if (!resolved) { const list = byBasename.get(target); if (list && list.length === 1) resolved = list[0]; }
      if (resolved) addEdge(fromId, resolved, "wikilink");
    }
  }

  const byGroup = new Map<string, string[]>();
  for (const id of ids) { const g = topGroup(id); const arr = byGroup.get(g) ?? []; arr.push(id); byGroup.set(g, arr); }
  for (const [, list] of byGroup) { list.sort(); for (let i = 0; i < list.length - 1; i++) addEdge(list[i], list[i + 1], "same-group"); }
  ...
  for (const [id, doc] of parsed) {
    ...
    for (const src of doc.sources) {
      ...
      const arr = bySource.get(base) ?? []; arr.push(id); bySource.set(base, arr);
    }
  }
  for (const [, list] of bySource) {
    if (list.length < 2) continue;
    const unique = Array.from(new Set(list)).sort();
    for (let i = 0; i < unique.length - 1; i++) addEdge(unique[i], unique[i + 1], "source-shared");
  }
...
function parseFm(content: string): { type?: string; title?: string; sources?: string[] } {
  ...
  for (const line of block.split("\n")) {
    ...
    if (t.startsWith("type:")) result.type = t.slice(5).trim();
      ...
      const arr = (result.sources as string[]) || [];
      arr.push(t.slice(2).trim());
      result.sources = arr;
    } else inList = false;
  }
  return { type: result.type as string | undefined, title: result.title as string | undefined, sources: result.sources as string[] | undefined };
}
